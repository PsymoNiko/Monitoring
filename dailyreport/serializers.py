...
class MentorReportSubmissionSerializer(serializers.ModelSerializer):
    student_name = serializers.CharField(source='student_name.full_name')

    class Meta:
        model = MentorReportSubmission
        fields = ('id', 'report_number', 'date', 'student_name', 'total_hours', 'delay', 'comment')

    def validate(self, data):
        """
        Check if the report is submitted before the deadline
        """
        deadline = data['date'].replace(hour=22, minute=0, second=0, microsecond=0)
        if data['date'] > deadline:
            raise serializers.ValidationError("Report is submitted after the deadline")
        return data
